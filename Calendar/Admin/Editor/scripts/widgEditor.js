...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		switch (widgToolbarItems[i])
		{
			...
			case "saveButton":
				this.addButton(this.theList.id + "ButtonSave", "widgButtonSave", "Save" , "save");
...
widgToolbar.prototype.addButton = function(theID, theClass, theLabel, theAction)
{
	...
	if(theClass == "widgButtonSave") {
		menuItem.className = 'widgButtonSave';
		menuItem.title = theLabel;
		menuItem.className = theClass;
		menuItem.action = theAction;
		menuItem.onclick = function() {
			alert("save Clicked!!");
		}
		menuItem.onmouseover = widgToolbarMouseover;
		menuItem.appendChild(theText);
