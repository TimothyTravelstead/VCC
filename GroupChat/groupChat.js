 ...
 * @returns {boolean} True if moderator, false otherwise
 */
function isCurrentUserModerator() {
	var flagElement = document.getElementById("groupChatModeratorFlag");
	if (!flagElement) return false;
	var flag = flagElement.value;
	return flag && flag !== "0";
}

/**
 * Handle errors securely - only show details to moderators, generic message to callers
 * Also sends error report via email
 * @param {string} errorMessage - The detailed error message
 * @param {string} context - Where the error occurred (e.g., "postMessage", "signOn")
 */
function handleGroupChatError(errorMessage, context) {
	// Log to console for debugging
	console.error("GroupChat Error [" + context + "]:", errorMessage);

	// Send error email (for all errors, regardless of user type)
	sendGroupChatErrorEmail(errorMessage, context);

	// Only show detailed error to moderators
	if (isCurrentUserModerator()) {
		alert("Error in " + context + ": " + errorMessage);
	} else {
		// Generic message for callers - no sensitive data
		alert("A temporary error occurred. Please try again. If the problem persists, please reload the page.");
	}
}

/**
 * Send error details via email for logging/monitoring
 * @param {string} errorMessage - The error message
 * @param {string} context - Where the error occurred
 */
function sendGroupChatErrorEmail(errorMessage, context) {
	try {
		var errorData = {
			context: context,
			error: errorMessage,
		...
		request.open("POST", "sendGroupChatError.php", true);
		request.setRequestHeader("Content-Type", "application/json");
		request.send(JSON.stringify({
			subject: "GroupChat Error: " + context,
...
/**
 * Clean up EventSource connections when page is closed/navigated away
 * This prevents orphaned PHP-FPM processes on the server
 */
window.addEventListener('beforeunload', function() {
	if (newChat && newChat.cleanup) {
		newChat.cleanup();
	}
});

// Also handle pagehide for mobile browsers (iOS Safari)
...
//	Section attempting to display and hide escape button based on whether its iframed or not.
...
function initPage() {
	...
	var params = "callerBrowser=" + encodeURIComponent(callerBrowser) + "&callerBrowserVersion=" + encodeURIComponent(callerBrowserVersion) + 
			"&callerOS=" + encodeURIComponent(callerOS) + "&callerOSVersion=" + encodeURIComponent(callerOSVersion) + "&callerBrowserDetail=" + 
			encodeURIComponent(callerBrowserDetail) + "&ChatRoomID=" + encodeURIComponent(chatRoomID) + "&userID=" + encodeURIComponent(userID);
...
function endChat() {
	// Close EventSource connection to prevent duplicates
	if(newChat && newChat.cleanup) {
		newChat.cleanup();
	}

	// Clear group chat state in parent window to allow system exit
	// Only attempt this for moderators to avoid cross-origin errors for regular chatters
	var groupChatModeratorFlag = document.getElementById("groupChatModeratorFlag").value;
	if(groupChatModeratorFlag !== "0" && groupChatModeratorFlag) {
		// Only for moderators - safe to access parent window (same origin)
	...
	// Clear the chat window to give visual feedback
	var groupChatMainWindow = document.getElementById("groupChatMainWindow");
	if(groupChatMainWindow) {
		groupChatMainWindow.innerHTML = "";
	}

	// Clear the members window
	var groupChatMembersWindow = document.getElementById("groupChatMembersWindow");
	if(groupChatMembersWindow) {
		groupChatMembersWindow.innerHTML = "";
	}

	// Reset the current user to allow re-login
	currentUser = null;
	users = {};
	messages = {};

	// Show sign-in form and hide chat controls
	var groupChatSignIn = document.getElementById("groupChatSignIn");
	if (groupChatSignIn) groupChatSignIn.style.display = null;
	...
	if(groupChatModeratorFlag !== "0" && groupChatModeratorFlag) {
		completeExitEmailTranscript();

		// For moderators, show a message that the room has been closed
		if(groupChatMainWindow) {
			var p = document.createElement("p");
			p.innerHTML = "You have successfully exited and closed the chat room.";
...
function completeExitEmailTranscript() {
	var chatRoomID = document.getElementById("groupChatRoomID").value;
	var completeExitEmailTranscriptResponseFunctionObject = {};
	var url = "Admin/includeFormatGroupChatEmail.php";
	var params = "chatRoomID=" + chatRoomID + "&userID=";

	var preFinalExitRequest = new AjaxRequest(url, params, completeExitEmailTranscriptResponseFunction , completeExitEmailTranscriptResponseFunctionObject);	

}

function completeExitEmailTranscriptResponseFunction(results) {
	if(results.trim() != "OK") {
		//alert("Exiting Results: " + results);
	} else {
		const nextStep = completeExit();
	}
}
...
// Global variable to prevent duplicate message submissions
...
function PostMessage(message) {
	...
	if(message.text.trim()) {
		...
		// Clear flag if no message to send
	...
	// Store the style elements for later use when message is successful
	window.pendingStyleElements = newElementStart + "<span contenteditable='true'>&nbsp</span>" + newElementEnd;

	// DON'T clear the message element here - only clear on successful response
...
function postMessageResult(results, resultObject) {
	// Always clear the submission flag when we get a response
	isSubmittingMessage = false;

	// DEBUG - Log when message POST completes
	console.log("[POST] Message saved at", new Date().toISOString(), "response:", results);
	
	if(results.trim() == "OK") {
		// Message was successful - clear the text area and apply stored styles
		var messageElement = document.getElementById("groupChatTypingWindow");
		if (messageElement) {
			messageElement.innerHTML = null;
			if (nicEditors && nicEditors.findEditor) {
				var editor = nicEditors.findEditor('groupChatTypingWindow');
				if (editor && window.pendingStyleElements) {
					editor.setContent(window.pendingStyleElements);
					window.pendingStyleElements = null; // Clear stored elements
				} else if (editor) {
					editor.setContent("<span contenteditable='true'>&nbsp</span>");
				}
			}
		}
		return true;
	} else if (results == "1") {
		// User is in timeout - show message but DO NOT clear text area
		createMessageBox("You are in a timeout for: " + results + " more minute.");
		return false;
	} else if (!isNaN(results) && results > 0) {
		// User is in timeout - show message but DO NOT clear text area
		createMessageBox("You are in a timeout for: " + results + " more minutes.");
		return false;
	} else if (results.includes("error") || results.includes("Error") || results.includes("Fatal")) {
		// Log error but don't show to user
		console.error("Server error in postMessageResult:", results);
		return false;
	}
...
function createMessageBox(message) {
	...
	// Add click event to the OK button to close the message box
	...
	// Add the overlay and message box to the document
...
function roomOpen() {
	...
	// CRITICAL: If current user is a moderator (groupChatModeratorFlag = 1 or 2), ALWAYS allow access
	// Moderators can ALWAYS sign in to open a room - NEVER show them "Room Closed"
	if(groupChatModeratorFlag && groupChatModeratorFlag != 0) {
		// User is a moderator - ALWAYS show the room as open
		isRoomOpen = true;
		...
		// Show sign-in form if not already signed on
	...
	} else if(loggedOnModerators == 0) {
		// Non-moderator user AND no moderators logged on - show "Room Closed"
		groupChatMainWindow.innerHTML = "";
		groupChatNumberCount.innerText = 0;
		if (groupChatSignIn) groupChatSignIn.style.display = "none";
		...
		// Non-moderator user BUT moderators are logged on - allow access
		isRoomOpen = true;
		var groupChatClosedRoomMessage = document.getElementById("groupChatClosedRoomMessage");
		if(groupChatClosedRoomMessage) {
			remove(groupChatClosedRoomMessage);
...
function GroupChatMonitor(name) {
	...
	this.init = function () {
		if(typeof(EventSource)!= "undefined") {
			// Add cache-busting to EventSource URL
			const cacheBuster = Date.now() + '_' + Math.floor(Math.random() * 1000000);
			var url = "chatFeed2.php?chatRoomID=" + self.chatRoomID + "&_cb=" + cacheBuster;
			console.log("[ES] Creating EventSource:", url);
			this.source = new EventSource(url);
			var source = this.source;

			source.addEventListener('open', function(event) {
				console.log("[ES] CONNECTED at", new Date().toISOString());
			});

			source.addEventListener('chatMessage', function(event) {
				console.log("[ES] MESSAGE at", new Date().toISOString());
				self.processChatMessage(event.data);
			});

			source.addEventListener('error', function(event) {
				console.log("[ES] ERROR readyState:", source.readyState);
			});
	...
	this.processChatMessage = function(message) {
		...
		switch (type) {
			...
			case "message":
				// Check by transaction id first (how messages are stored) to prevent duplicates
				if(messages[data.id]) {
					messages[data.id].updateMessage(data);
				} else if(data.messageNumber && messages[data.messageNumber]) {
			...
			case 'timeout':
				if(users[data.userID] == users[currentUser]) {
					const minutes = data.minutes;
					if(minutes == 1) {
						createMessageBox("You are in a timeout for: " + minutes + " minute.");
					} else {
						createMessageBox("You are in a timeout for: " + minutes + " minutes.");
					}
				}
				break;
		}
...
function confirmedCloseCheck(chatRoomID) {
	console.log("========== confirmedCloseCheck() DEBUG ==========");
	var moderators = countLoggedOnUsers();
	console.log("moderators count:", moderators);
	if(moderators > 0) {
		console.log("Moderators present, NOT closing room");
		console.log("=================================================");
		return;
	}
	console.log("No moderators, calling roomClosed()");
	console.log("=================================================");
...
function roomClosedCheck(chatRoomID) {
	console.log("========== roomClosedCheck() DEBUG ==========");
	console.log("Setting 3 second timeout before calling confirmedCloseCheck()");
	console.log("chatRoomID:", chatRoomID);
	console.log("=============================================");
  	myVar = setTimeout(function(){ confirmedCloseCheck(chatRoomID); }, 3000);
...
function roomClosed(chatRoomID) {
	...
	var groupChatModeratorFlag = document.getElementById("groupChatModeratorFlag").value;
	console.log("groupChatModeratorFlag:", groupChatModeratorFlag);

	if(groupChatModeratorFlag === "0" || !groupChatModeratorFlag) {
		console.log("User is NOT a moderator - showing room closed message");
		var groupChatSignIn = document.getElementById("groupChatSignIn");
		if (groupChatSignIn) groupChatSignIn.style.display = "none";
		...
		if(originalPage) {
			...
			window.location.href = "https://www.lgbthotline.org/youthchatrooms.html";
		}


	} else {
		console.log("User IS a moderator");
		console.log("CRITICAL: roomClosed() was called but user is a moderator");
		console.log("This should NOT reset selector/iframe - moderators should stay connected");
		console.log("Preventing selector reset and iframe clear");

		// DO NOT reset selector or clear iframe for moderators
		// Moderators can close a room administratively but should remain connected
		// The old code was resetting the selector/iframe which kicked moderators out
...
function Message(message) {
	...
	this.postUpdateToMessageResponse = function(responseText , responseObject) {
		if(responseText != "OK") {
			handleGroupChatError(responseText, "postUpdateToMessage");
		}
	};
...
function countLoggedOnUsers() {
	console.log("========== countLoggedOnUsers() DEBUG ==========");
	var chatRoomID = document.getElementById("groupChatRoomID").value;
	var loggedOnUsers = 0;
	var moderators = 0;
	...
	for (var key in users) {
		var user = users[key];
		console.log("User:", key, "status:", user.status, "moderator:", user.moderator);
		if(user.status) {
			loggedOnUsers += 1;
		}
		...
		if(user.moderator == 2 || (user.moderator && user.status)) {
			console.log("Counting as moderator:", key);
			moderators += 1;
		}
	}
	var groupChatNumberCount = document.getElementById("groupChatNumberCount");
	...
	console.log("Total moderators:", moderators);
	console.log("================================================");
	return moderators;
}
...
function User(message) {
	...
	this.updateUser = function(message) {
		console.log("========== User.updateUser() DEBUG ==========");
		...
		if(self.moderator == 2 && self.userID == users[currentUser].userID) {
			console.log("Moderator = 2 and currentUser - calling logOnUser()");
			var groupChatTypingWrapper = document.getElementById("groupChatTypingWrapper");
			var groupChatSignIn = document.getElementById("groupChatSignIn");
			self.logOnUser();
			groupChatTypingWrapper.style.display = "none";
			...
			console.log("ACTION = SIGNOFF - calling logOffUser()");
			self.signedOn = 0;
			self.status = 0;
			self.logOffUser();
		} else if(message.action === "signon") {
			console.log("ACTION = SIGNON - calling logOnUser()");
			self.signedOn = 1;
			self.status = 1;
			self.logOnUser();
			self.display();
	...
	this.addModeratorOptionsWindow = function(chatAvailableFlag) {
		...
		var userTimeoutToday = document.createElement("input");
		userTimeoutToday.style.display = "block";
		userTimeoutToday.style.fontSize = "12px";
		userTimeoutToday.type = "button";
		userTimeoutToday.value = "User Timeout";
		userTimeoutToday.style.margin = "10px";
		
		userTimeoutToday.onclick = self.addModeratorTimoutOptionsWindow;

		select.appendChild(userTimeoutToday);
	...
	this.addModeratorTimoutOptionsWindow = function() {
		//Remove main Moderator Options Window
		var existingOptionsWindow = document.getElementById("groupChatModeratorUserOptions");
		if (existingOptionsWindow) {
			remove(existingOptionsWindow);
		...
		//Remove any Old Block Options Windows
		var existingTimeoutOptionsWindow = document.getElementById("groupChatModeratorTimeoutOptions");
		if (existingTimeoutOptionsWindow) {
			remove(existingTimeoutOptionsWindow);
		}
		
		//Create New Timeout Options Window
		var select = document.createElement("div");
		select.style.position = "absolute";
		select.setAttribute("class","div");
		select.style.width = "auto";
		select.id = "groupChatModeratorTimeoutOptions";
		var h1 = document.createElement("h3");
		h1.appendChild(document.createTextNode("User Timeout"));
		select.appendChild(h1);

		var label = document.createElement("label");
		label.appendChild(document.createTextNode("Timeout Minutes:"));
		select.appendChild(label);

		var userTimeoutMinutes = document.createElement("input");
		userTimeoutMinutes.style.display = "inline-block";
		userTimeoutMinutes.style.fontSize = "12px";
		userTimeoutMinutes.style.width = "40px;";
		userTimeoutMinutes.style.marginLeft = "20px";
		userTimeoutMinutes.type = "number";
		userTimeoutMinutes.style.margin = "10px";

		select.appendChild(userTimeoutMinutes);

		var option = document.createElement("input");
		option.type = 'button';
		option.style.fontSize = "12px";
		option.style.fontWeight = "bold";
		option.style.marginLeft = "50px";
		option.value = "CANCEL";
		option.class = "defaultButton";
		option.style.padding = "5px";
		option.onclick = function() {
		...
		var timeoutButton = document.createElement("input");
		timeoutButton.style.display = "inline-block";
		timeoutButton.style.fontSize = "12px";
		timeoutButton.type = "button";
		timeoutButton.value = "Timeout";
		timeoutButton.style.marginLeft = "10px";
		
		timeoutButton.onclick = function() {  //Block Caller Code
			self.timeoutThisUser(userTimeoutMinutes.value);
			remove(this.parentNode);
			self.recolorBlock();
		};
		
		select.appendChild(timeoutButton);
	...
	this.sendThisUserToChatResults = function(results, responseObject) {
		if(results != "OK") {
			handleGroupChatError(results, "sendThisUserToChat");
		}
	};
	...
	this.blockUserResults = function(results, responseObject) {
		if(results != "OK") {
			handleGroupChatError(results, "blockUser");
		}
	};

	this.timeoutThisUser = function(timeoutMinutes) {
		var chatRoomID = document.getElementById("groupChatRoomID").value;
		var params = "chatRoomID=" + chatRoomID + "&minutes=" + timeoutMinutes + "&userID=" + self.userID + "&name=" + self.name;
		var responseObject = {};
		var url= "Admin/timeoutUser.php";
		var blockUserRequest = new AjaxRequest(url, params, self.timeoutThisUserResults , responseObject);
	};

	this.timeoutThisUserResults = function(results, responseObject) {
		if(results != "OK") {
			handleGroupChatError(results, "timeoutUser");
		}
	};
...
//		self.imMessage.value = "";
	...
	this.postIM = function (user) {
		...
		params = "imTo=" + encodeURIComponent(self.userID) + "&userID=" + encodeURIComponent(users[currentUser].userID) + "&Text=" + imMessageText + "&chatRoomID=" + chatRoomID;
...
function groupChatLogin() {
	...
	if (!loginForm) return false; // No login form available for admin moderators
	...
	var nameElement = document.getElementById("groupChatSignInFormName");
	if (!nameElement) return false; // Form not available
	...
	var params = "Name=" + encodeURIComponent(name) + "&Avatar=" + encodeURIComponent(selectedAvatar.getAttribute("src")) + 
...
function signOnResponse(results, resultObject) {
	if(results == "OK") {
		...
		// Use secure error handler - only moderators see details
		handleGroupChatError(results, "signOn");
	}
}
