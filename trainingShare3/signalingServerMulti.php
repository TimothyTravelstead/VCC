...
// Get room ID and participant info directly from URL parameters
...
if (count($_POST) != 0 || $_SERVER['REQUEST_METHOD'] === 'POST') { 
    ...
    error_log("SIGNALING DEBUG: Received POST data: " . $posted);
    
    // Try to decode as JSON first
    $signal = json_decode($posted, true);
    error_log("SIGNALING DEBUG: JSON decode result: " . ($signal ? "SUCCESS" : "FAILED"));
    ...
    error_log("SIGNALING DEBUG: Signal processing completed for type: " . $signal['type']);
    echo "OK";
...
function broadcastToRoom($message, $roomFile, $senderId) {
    ...
    error_log("SIGNALING DEBUG: Broadcasting to room - found " . count($room['participants']) . " participants");
    
    foreach ($room['participants'] as $participantId => $participant) {
        if ($participantId !== $senderId) {
            error_log("SIGNALING DEBUG: Broadcasting to participant: $participantId");
            sendToParticipant($message, $participantId);
        } else {
            error_log("SIGNALING DEBUG: Skipping sender: $participantId");
        }
    }
}
...
function sendToParticipant($message, $participantId) {
    ...
    if (file_exists($participantFile) && filesize($participantFile) > 0) {
        file_put_contents($participantFile, '_MULTIPLEVENTS_' . $messageJson, FILE_APPEND | LOCK_EX);
    } else {
        file_put_contents($participantFile, $messageJson, LOCK_EX);
    }
    
    error_log("SIGNALING DEBUG: Successfully wrote signal to file: $participantFile");
}
