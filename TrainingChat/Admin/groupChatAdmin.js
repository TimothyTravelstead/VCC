...
window.onload = function() {
	...
	var adminMonitor = document.getElementById("adminMonitor").value.trim();  //Value of 2 = Admin; 1 = Moderator
	const VCCIMButtonElement = document.getElementById("VCCIMButton");
	
	if(adminMonitor == 2) {    	// If Admin, not Monitor, show close room buttons & hide VCCIm Functions
...
//		VCCIMButtonElement.onclick = function() {VCCIMButton(this);};
	...
	groupChatMonitor1CloseButton.onclick = function() {
		closeRoom(groupChatMonitor1RoomSelector.selectedIndex);
	}
	
	groupChatMonitor2CloseButton.onclick = function() {
		closeRoom(groupChatMonitor2RoomSelector.selectedIndex);
	}
...
function closeRoom(room) {
	...
	var verifyClose = confirm("Close the room: " + groupChatMonitor1RoomSelector.options[room].innerHTML + "?");
	if(verifyClose) {	
		var responseObject = {};
		var url = "closeRoom.php";
		var params = "chatRoomID=" + room;
...
function moderatorSignOn(frame, selector) {
	...
	frame.src = '../index.php?ChatRoomID=' + selector + '&userID=' + userID;
...
function VCCMonitor(name) {
	...
	this.init = function () {
		if(typeof(EventSource)!=="undefined") {
			...
			const adminMonitor = document.getElementById("adminMonitor").value.trim();  //Value of 2 = Admin; 1 = Moderator
