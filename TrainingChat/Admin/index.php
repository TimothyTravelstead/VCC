...
function vccLoggedOnStatus($userID) {
	...
	$queryA = "select LoggedOn FROM Volunteers WHERE UserName = '".$userID."'";
	$resultA = mysqli_query($connection, $queryA);

	$result_row = mysqli_fetch_row(($resultA));
	$status = $result_row[0];
	return $status;	
}
...
$params = [];	
$query = "Select id, name, url from groupChatRooms WHERE open = 1";
...
$adminMonitor = vccLoggedOnStatus($userID);
...
<head>
	...
	<script src="groupChatAdmin.js" type="text/javascript"></script>
