...
function showTimelineError(message, errorObject) {
	...
	console.group('🚨 TIMELINE ERROR - DETAILED DIAGNOSTICS');
	...
	`;
	header.innerHTML = `
		<span>🚨 Timeline Error - Comprehensive Diagnostics</span>
...
window.addEventListener('error', function(event) {
	...
	errorDetails += '═══════════════════════════════════════\n';
	errorDetails += '💡 TIP: Check browser console for more details\n';
	errorDetails += '═══════════════════════════════════════\n';
...
window.addEventListener('unhandledrejection', function(event) {
	...
	errorDetails += '═══════════════════════════════════════\n';
	errorDetails += '💡 TIP: This is an async error. Check browser console.\n';
	errorDetails += '═══════════════════════════════════════\n';
		...
		try {
			if(!resultObject.results) {
				...
				try {
					...
					errorDetails += '📦 SERVER RESPONSE (first 500 chars):\n';
			...
			// Check if helpline was closed on this day
			if (!resultObject.hours.openTime || !resultObject.hours.closeTime) {
				// Helpline was closed on this day (e.g., Sunday)
				var body = document.getElementById("timelineGraph");
				body.innerHTML = '<div style="padding: 40px; text-align: center; font-size: 24px; color: #666;">' +
					'<h2>Helpline Closed</h2>' +
					'<p>The helpline was not operating on this day.</p>' +
					'</div>';

				// Hide volunteer table
				var volunteerTable = document.getElementById("volunteerTable");
				if (volunteerTable) {
					volunteerTable.innerHTML = '<tr><td colspan="4" style="text-align: center; padding: 20px;">No volunteers scheduled</td></tr>';
				}

				return; // Exit early without processing call data
			}
...
function graphCallData() {
	...
	for (const [key, value] of Object.entries(sortable)) {
		...
		// Show comprehensive error modal for any graphCallData errors
		var errorDetails = '═══════════════════════════════════════\n';
		errorDetails += '      TIMELINE RENDERING ERROR\n';
		errorDetails += '═══════════════════════════════════════\n\n';
		errorDetails += '🔴 ERROR:\n';
		errorDetails += '   ' + error.message + '\n\n';
		errorDetails += '📍 LOCATION:\n';
		errorDetails += '   File: timeline.js\n';
		errorDetails += '   Function: graphCallData()\n\n';
		if (error.stack) {
			errorDetails += '📋 STACK TRACE:\n';
			var stackLines = error.stack.split('\n').slice(0, 10);
			stackLines.forEach(function(line) {
		...
		errorDetails += '═══════════════════════════════════════\n';
		errorDetails += '💡 TIP: Check browser console for details\n';
		errorDetails += '═══════════════════════════════════════\n';

		showTimelineError(errorDetails, error);
		throw error; // Re-throw so it's also caught by global handler
	}
}
